...
no system library installation required. Users need only `go get`.
...
1. Edit the version tag in the `//go:generate` directive in `talib.go`.
2. Run `go generate ./...` from the repository root.
3. Commit `include/`, `talib_amalgamation.c`, and `*_gen.go` files.
